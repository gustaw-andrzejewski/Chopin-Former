...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_dim: int, head_dim: int, num_heads: int, num_layers: int, max_seq_len: int, dropout: float = 0.1):
        super().__init__()
    ...
    def generate(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Initializing the generated sequence with the context
        generated_sequence = x
        # Iterating over the maximum sequence length
        for _ in range(self.max_seq_len):
            # Getting the last token of the generated sequence
            x = generated_sequence[:, -1:]
            # Getting the logits for the last token
            logits = self.forward(x)
            # Getting the probabilities for the last token
            probs = F.softmax(logits, dim=-1)
            # Sampling from the vocabulary using the probabilities
            next_token = torch.multinomial(probs[:, -1], num_samples=1)
            # Appending the sampled token to the generated sequence
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, max_seq_len:int, embedding_dim:int, head_dim:int, num_heads:int):
        ...
        self.heads = nn.ModuleList([
            AttentionHead(max_seq_len, embedding_dim, head_dim) for _ in range(num_heads)
        ])
